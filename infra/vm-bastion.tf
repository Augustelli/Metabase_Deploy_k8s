resource "openstack_compute_instance_v2" "k8s_bastion" {
  name              = "vm-bastion"
  image_id          = data.openstack_images_image_v2.ubuntu_2204.id
  flavor_id         = data.openstack_compute_flavor_v2.small.id
  key_pair          = var.key_name
  security_groups   = [openstack_compute_secgroup_v2.k8s_sg_bastion.name]
...
resource "openstack_compute_floatingip_associate_v2" "tf_bastion_fip" {
  ...
  instance_id = openstack_compute_instance_v2.k8s_bastion.id
}
